<template>
  <teleport to='body'>
    ...
    <div ref="target" class="bg-transparent rounded-xl shadow fixed top-0 left-0 flex flex-col text-start items-start" :style="pos">
      <button class="[&:nth-child(1)]:rounded-b-none [&:last-child]:rounded-t-none btn btn-sm bg-neutral hover:bg-neutral-focus text-base text-start flex-1 w-32 justify-start" v-for="a in actions" :key="a.name" @click="a.fn">{{ a.name }}</button>
    </div>
  </teleport>
</template>
